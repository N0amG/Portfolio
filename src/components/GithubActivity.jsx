export default function GithubActivity({ events = [], titre, className }) {
	...
	function getFirstWordColor(firstWord) {
		...
		if (word.includes('add') || word.includes('create'))
			return 'bg-green-600'
		if (word.includes('fix') || word.includes('optimiz'))
			return 'bg-orange-400'
		if (word.includes('remove') || word.includes('delete'))
			return 'bg-red-600'
		if (word.includes('update') || word.includes('upgrade'))
			return 'bg-blue-600'
			...
			<ul className='github-activity-list flex flex-1 flex-col gap-4 p-2 h-full '>
				{events.map((event) => {
					...
					return (
						<li
							key={event.id}
							className='github-commit-item bg-slate-800/70 rounded-lg p-4 shadow border border-slate-700 flex flex-col gap-2'
						>
							...
							<div className='flex items-start gap-2'>
								...
								<span className='text-slate-200 font-mono text-sm break-words whitespace-pre-line'>
									{messageSansFirstWord}
								</span>
							</div>
							{event.repo?.name && (
								<a
									href={`https://github.com/N0amG/${event.repo?.name}`}
									target='_blank'
									rel='noopener noreferrer'
									className='text-rose-300 hover:underline text-xs mt-1 w-fit'
								>
									Voir sur GitHub
								</a>
							)}
