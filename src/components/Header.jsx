	...
	return (
		<header className='w-full text-slate-400 select-none z-10 border-b-2 border-slate-800'>
			<div className='flex items-center justify-between w-full h-16'>
				<div className={`flex items-center w-1/5 max-w-[300px] h-full px-6 ${borderXClass} border-slate-800 text-lg text-left`}>
					noamg-guez
				</div>
				<nav className='flex-1 flex h-full w-[66.6667%]'> {/* 2x w-1/3 = w-2/3 */}
					<ul className='flex items-center h-full divide-slate-800 border-slate-800 w-full'>
						<li className={`px-8 h-full flex items-center relative group ${borderXClass} cursor-pointer transition-colors ${pathname === "/" ? focusStyle : ""}`}
						...
						<li className={`px-8 h-full flex items-center ${borderXClass} cursor-pointer transition-colors ${pathname === "/about" ? focusStyle : ""}`}
							onMouseEnter={e => e.currentTarget.classList.add("text-slate-50")}
							onMouseLeave={e => { if (pathname !== "/about") e.currentTarget.classList.remove("text-slate-50") }}>
							<Link href="/about">_about-me</Link>
						</li>
						<li className={`px-8 h-full flex items-center ${isAbout ? 'border-x-transparent' : 'border-x border-slate-800'} cursor-pointer transition-colors ${pathname === "/projects" ? focusStyle : ""}`}
							onMouseEnter={e => e.currentTarget.classList.add("text-slate-50")}
				...
				<div className={`flex items-center h-full px-6 border-slate-800 border-l-2 cursor-pointer hover:text-primary transition-colors ${pathname === "/contact" ? focusStyle : ""}`}>
